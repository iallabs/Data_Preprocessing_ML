...
num_input = 3072 #  data input (img shape: 256*256)
num_classes = 2 # 
...
tfrec_dev_input_fn = trec.decode_tfrecord.tfrec_data_input_fn(["catvdog.tfrecords"])
...
with tf.Session() as sess:
    ...
    print("Testing Accuracy:", \
            sess.run(accuracy, feed_dict={X: img,
                                      Y: label}))
    '''tfrec_dcatvdog= trec.decode_tfrecord.tfrec_data_catvdog(["catvdogtest2.tfrecords"])
    a = tfrec_dcatvdog()
    for i in range(750):
