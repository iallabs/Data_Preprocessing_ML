    ...
    with tf.device('/cpu:0'):
        for spec in range(len(spec_list)):
            with tf.name_scope('weights'):
                W = tf.get_variable("W"+str(spec+1),[spec_list[spec][0],spec_list[spec][1],spec_list[spec][2],spec_list[spec][3]], initializer=tf.random_normal_initializer(seed=0))
