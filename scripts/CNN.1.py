...
def forward_propagation(X, parameters):
    ...
    # RELU
    A1 = tf.nn.relu(Z1)
    # MAXPOOL: window 7x7, sride 7, padding 'SAME'
    P1 = tf.nn.max_pool(A1, ksize = [1,4,4,1], strides = [1,4,4,1], padding = 'SAME')
    # CONV2D: filters W2, stride 1, padding 'SAME'
    Z2 = tf.nn.conv2d(P1, W2, strides=[1,1,1,1], padding='SAME')
    # RELU
    A2 = tf.nn.relu(Z2)
    # MAXPOOL: window 5x5, stride 5, padding 'SAME'
    ...
    A4 = tf.nn.relu(Z4)
    # MAXPOOL: window 3x3, stride 3, padding 'SAME'
    P4 = tf.nn.max_pool(A4, ksize=[1,3,3,1], strides=[1,3,3,1], padding='SAME')
    # CONV2D: filters W4, stride 1, padding 'SAME'
    Z5 = tf.nn.conv2d(P4, W5, strides=[1,1,1,1], padding='SAME')
    # RELU
    A5 = tf.nn.relu(Z5)
    # MAXPOOL: window 3x3, stride 3, padding 'SAME'
    P5 = tf.nn.max_pool(A5, ksize=[1,3,3,1], strides=[1,3,3,1], padding='SAME')
    # FLATTEN
